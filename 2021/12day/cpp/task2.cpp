...
void walk_graph(const std::multimap<std::string, std::string>& graph,
    ...
    for (auto i = neigh_range.first; i != neigh_range.second; i++) {
        if (i->second == "end") {
            ...
            if (std::find(path.begin(), path.end(), i->second) == path.end()) {
                auto l2 = path;
                l2.push_back(i->second);
                walk_graph(graph, l2, global_paths, credit);
            } else if (credit) {
                auto l2 = path;
                l2.push_back(i->second);
                walk_graph(graph, l2, global_paths, false);
            }
        } else {
            auto l2 = path;
            l2.push_back(i->second);
...
int main(int argc, char* argv[]) {
    ...
    walk_graph(graph, start_path, global_paths, true);
