...
void print(const std::vector<std::vector<int>>& diffs, const std::vector<int>& fronts) {
...
int main(int argc, char* argv[]) {
    ...
    for (std::string line; getline(input, line); ) {
        ...
        for (int j = static_cast<int>(diffs.size() - 1); j >= 0; j--) {
            fronts[j] = diffs[j].front() - base;
            base = fronts[j];
        }
        estimates.push_back(fronts[0]);
        print(diffs, fronts);
    }
