...
vector<int> bfs(vector<vector<int>> g, int initial) {
...
pair< map<string,int>, vector<vector<int>> > read_input(int& start, int& end) {
    map<string, int> ids{};
...
int main() {
    ...
    vector<vector<int>> graph;
    int initial, santa;
    auto result = read_input(initial, santa);
    cout << "Graph size: " << result.second.size() << std::endl;
    ids = result.first;
    graph = result.second;
    vector<int> distances = bfs(graph, initial);
    int total_sum = 0;
    for (int d: distances) {
        total_sum += d;
    }
    cout << "Total distance: " << total_sum << std::endl;
    cout << "Santa's distance: " << distances[santa] << std::endl;
    return 0;
}
