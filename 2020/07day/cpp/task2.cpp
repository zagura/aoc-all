...
int process_graph_task2(const std::map<std::string, Node>& graph, const std::string& search) {
    int total_bags = 0;
    try {
        std::string current = search;
        for (const auto& node: graph.at(current).contain) {
            total_bags += node.second * process_graph_task2(graph, node.first);
        }
    }  catch (std::out_of_range& out) {
        std::cerr << "Problem: " << out.what() << std::endl;
    }
...
int main(int argc, char* argv[]) {
    ...
    int result = process_graph_task1(graph, my_bag) - 1;
    ::printf("Task 1 result: %d\n", result);
    int result_task2 = process_graph_task2(graph, my_bag);
