...
int main(int argc, char* argv[]) {
    ...
    std::map<std::pair<size_t, size_t>, std::vector<int>> gears;
    for (auto& row: nums) {
        for (auto& n: row) {
            for (auto& s: n.symbols) {
                if (s.c == '*') {
                    auto coords = std::make_pair(s.x, s.y);
                    if (!gears.contains(coords)) {
                        gears[coords] = std::vector<int> {};
                    }
                    gears[coords].push_back(n.value);
                }
            }
        }
    }
    uint64_t task2_result = 0;
    for (auto& [gear_loc, parts]: gears) {
        if (parts.size() == 2) {
            uint64_t ratio = parts[0] * parts[1];
            std::cout << std::format("Grear at ({},{}): ({}, {}) -> {}\n", gear_loc.first, gear_loc.second,
                                     parts[0], parts[1], ratio);
            task2_result += ratio;
        }
    }
