...
int main(int argc, char* argv[]) {
    ...
    for (int step = 0; step < connections && edge != edges.end(); edge++) {
        auto [l, r] = edge->ids;
        ...
        if (fu[l] == fu[r]) {
            ...
            auto new_lsize = std::count_if(fu.begin(), fu.end(), [&](const auto& el) { return el == left; });
            auto new_rsize = std::count_if(fu.begin(), fu.end(), [&](const auto& el) { return el == right; });
            std::println("Merging two sets of size: {} and {} into {} and {}", lsize, rsize, new_lsize, new_rsize);

        }
    }
    // for (int i = 0; i < static_cast<int>(fu.size()); i++) {
    //     if (fu[i] != i) {
    //         for (int j = i + 1; j < static_cast<int>(fu.size()); j++) {
    //             if (fu[j] == i) {
    //                 fu[j] = fu[i];
    //             }
    //         }
