...
// bool high_cards_with_swap(std::array<int, 5> cards) {
//     for (size_t i = 4; i > 0; i--) {
//         if (cards[i] - cards[i - 1] != 1) {
//             if (cards[i] == 1) {
//                 cards[i] = cards[i - 1] + 1;
...
int get_type(std::string cards) {
    ...
    if (values.contains(5)) {
        ...
        if (values.contains(2)) {
            if (counts.at(1) == 3 || counts.at(1) == 2) {
                return 7;
            }
            return 5;
        } else {
            if (counts.at(1) == 1 || counts.at(1) == 3) {
                return 6;
            }
            return 4;
        }
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < game.size(); i++) {
        auto d = game[i].first;
        std::cout << std::format("Hand {:03} - {}: type ({}), strength ({:x})\n", i, d.cards, d.type, d.strength);
