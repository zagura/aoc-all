...
//                printf("Board %d: %d matched %d: sum %d\n", id, board_sum, number, matched_sum);
                bool old_won = won;
                if ((matched_rows[row] == count)
                    || (matched_columns[column] == count)) {
                    won = true;
                }
                // Won only counted on first time
                return (not old_won && won);
            }
        }
    }
    return 0;
...
int match_elements(std::vector<board>& boards, int number, size_t& win_count) {
    for (auto& b: boards) {
        if (b.match(number)) {
            win_count++;
...
int main(int argc, char* argv[]) {
    ...
    for (auto number: history) {
        score = match_elements(boards, number, win_count);
        if (score) {
            break;
        }
    }
