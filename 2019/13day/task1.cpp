...
int main() {
    ...
    try {
        ...
        size_t tiles = 0;
        for (size_t index = 2; index < processor.output.size(); index += 3) {
            if (processor.output[index] == 2) {
                // block tile
                tiles++;
            }
        }
        cout << "Task 1 result: " << tiles << endl;
