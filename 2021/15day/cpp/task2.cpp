...
int main(int argc, char* argv[]) {
    ...
    // Resize data_map
    size_t orig_X = data_map.size();
    size_t orig_y = data_map[0].size();
    data_map.resize(5 * data_map.size());
    for (auto& row: data_map) {
        row.resize(5 * orig_y);
    }
    for (auto dx: {0, 1, 2 ,3, 4}) {
        for (auto dy: {0, 1, 2, 3, 4}) {
            for (size_t x = 0; x < orig_X; x++) {
                for (size_t y = 0; y < orig_y; y++) {
                    auto val = data_map[x][y] + dx + dy;
                    val %= 9;
                    if (val == 0) {
                        val = 9;
                    }
                    data_map[(dx * orig_X) + x][(dy * orig_y) + y] = val;
                }
            }
        }
    }
