...
for p in range(16):
    for i in range(16):
        if len(matching[i]) == 1:
            found[i] = list(matching[i].keys())[0]
            name = found[i]
            for n in range(16):
                if name in matching[n]:
                    del matching[n][name]
...
state = State('[0, 0, 0, 0]')
for k in found:
    found[k] = operations[found[k]]

for l in lines[total*4:]:
    l = l.split()
    if len(l) == 4:
        instruction = Instruction(l)
        state = found[instruction.opcode](state, instruction)
        # print("Instruction: {}\t State after: {}".format(instruction, state))
print(state)
