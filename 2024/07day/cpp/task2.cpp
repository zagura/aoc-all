...
int64_t concat(const int64_t left, const int64_t right) {
    return std::stoull(std::format("{}{}", left, right));
}
...
bool evaluate(const std::vector<int64_t>& numbers, size_t pos, const int64_t expected, const int64_t current_value) {
    ...
    return evaluate(numbers, pos + 1, expected, current_value * numbers[pos])
           || evaluate(numbers, pos + 1, expected, current_value + numbers[pos])
           || evaluate(numbers, pos + 1, expected, concat(current_value, numbers[pos]));
}
