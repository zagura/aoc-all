...
struct Task {
    std::pair<int, int> a_move;
    std::pair<int, int> b_move;
    std::pair<int, int> prize;

    size_t find_path();
    std::optional<size_t> move_a(Point s, size_t current_cost);
    std::optional<size_t> move_b(Point s, size_t current_cost);
};

size_t Task::find_path() {
...
std::optional<size_t> Task::move_a(Point s, size_t current_cost) {
...
std::optional<size_t> Task::move_b(Point s, size_t current_cost) {
    if (s.first > prize.first || s.second > prize.second) {
        return {};
    } else if (s == prize) {
        return { current_cost };
...
std::pair<int, int> get_move(const std::string& token) {
    auto coords = split_line(token, ',');
    int dx = std::stoi(coords[0].substr(3));
    int dy = std::stoi(coords[1].substr(3));
    return { dx, dy };
}

std::pair<int, int> get_prize(const std::string& token) {
    auto coords = split_line(token, ',');
    int x = std::stoi(coords[0].substr(3));
    int y = std::stoi(coords[1].substr(3));
    return { x, y };
}
...
int main(int argc, char* argv[]) {
    ...
    for(auto& task: tasks) {
        size_t path_cost = task.find_path();
