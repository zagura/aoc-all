...
vector<Point> get_crosspoints(const vector<Point>& path1, const vector<Point>& path2) {
    ...
    for (size_t i1 = 0; i1 < path1.size() - 1; i1 += 1) {
        ...
        for (size_t i2 = 0; i2 < path2.size() - 1; i2 += 1) {
            ...
            if (cross) {
               crosspoints.push_back(cross.value());
            }
        }
    }
    return crosspoints;
...
int main() {
    ...
    print_path(first);
    print_path(second);
    vector<Point> crosspoints = get_crosspoints(first, second);
    std::sort(crosspoints.begin(), crosspoints.end(), [](Point p1, Point p2) {
        return ((abs(p1.x) + abs(p1.y)) < (abs(p2.x) + abs(p2.y)));});
    cout << "\nFinal crosspoints: " <<  '\n';
    for (auto& cp: crosspoints) {
        cout << cp.x << ',' << cp.y << ":  " << abs(cp.x) + abs(cp.y) << std::endl;
    }
    return 0;
}
